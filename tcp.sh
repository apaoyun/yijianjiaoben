...
optimizing_system(){
	...
	sed -i '/net.core.netdev_max_backlog/d' /etc/sysctl.conf
	sed -i '/net.ipv4.tcp_timestamps/d' /etc/sysctl.conf
	sed -i '/net.ipv4.tcp_max_orphans/d' sysctl.conf
	sed -i '/net.ipv4.ip_forward/d' /etc/sysctl.conf
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "http://www.78idc.cn/tcp.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -N --no-check-certificate http://www.78idc.cn/tcp.sh && chmod +x tcp.sh
...
#一键国内三网测速
cs(){
bash <(curl -Lso- http://www.78idc.cn/speedtest.sh)
}

#Linux 一键更换国内软件源
rjy(){
#sudo bash < <(wget http://www.78idc.cn/ChangeMirrors.sh && bash ChangeMirrors.sh)
#bash <(curl -sSL https://gitee.com/SuperManito/LinuxMirrors/raw/main/ChangeMirrors.sh)
bash <(curl http://www.78idc.cn/ChangeMirrors.sh)
}

#一键优化脚本 一键更换阿里云yum源/EPEL源         快捷命令./yum.sh
yum(){
bash <(curl http://www.78idc.cn/yum.sh)
}

#Linux 服务器测试脚本 快捷命令 ./box.sh 
fwqcs(){
bash <(curl http://www.78idc.cn/box.sh)
}
...
echo && echo -e " 阿炮云TCP加速 一键安装管理脚本 快捷命令./tcp.sh ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
 ${Green_font_prefix}01.${Font_color_suffix} 系统信息查询
